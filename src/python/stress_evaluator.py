...
class StressEvaluator(ABC):
    ...
    @abstractmethod
    def compute_stress(
        self, cell_structure: "Cell", potential: "Potential"
    ) -> np.ndarray:
        """
        @brief 计算晶体结构的应力张量。
...
class EAMStressEvaluator(StressEvaluator):
    ...
    def compute_stress(
        ...
        # positions = np.array([p.position for p in cell_structure.atoms])
        # stress_voigt = calculate_stress_fortran(positions, cell_structure.volume, potential.parameters, potential.cutoff)
        # return stress_voigt
