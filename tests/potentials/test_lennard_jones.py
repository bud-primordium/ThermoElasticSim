...
def two_atom_cell():
    ...
    atom2 = Atom(id=2, symbol='Ar', mass_amu=39.948, position=np.array([3.8, 0, 0])) # 略大于sigma
...
def test_lj_force_calculation(lj_potential, two_atom_cell):
    ...
    # 验证力的方向和大小
    # 力应该沿着原子间连线方向
    assert np.abs(force_on_atom1[0]) == pytest.approx(expected_force_magnitude, rel=1e-5)
...
def test_lj_pybind_ctypes_energy_consistency(two_atom_cell):
    ...
    positions = np.ascontiguousarray(two_atom_cell.get_positions(), dtype=np.float64).flatten()
    box_lengths = np.ascontiguousarray(two_atom_cell.get_box_lengths(), dtype=np.float64)
    pairs = [(i, j) for i in range(num_atoms) for j in nl.get_neighbors(i) if j > i]
    neighbor_list_array = np.ascontiguousarray([k for ij in pairs for k in ij], dtype=np.int32)
    num_pairs = len(pairs)
