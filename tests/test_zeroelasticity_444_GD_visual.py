# 文件名: test_zeroelasticity_444_GD_visual.py
# 作者: Gilbert Young
# 修改日期: 2024-10-20
...
def configure_logging():
    ...
    # 日志文件路径
    log_directory = f"./output/test_444_GD_{current_time}"
...
def test_zeroK_elastic_constants(configure_logging):
    ...
    elastic_calculator = ZeroKElasticConstantsCalculator(
        ...
        delta=1e-3,
        optimizer_type="GD",  # 使用BFGS优化器
        save_path=log_directory,  # 将测试输出路径传入计算器
    )
    logger.debug("Initialized ZeroKElasticConstantsCalculator with GD Optimizer.")
