...
def test_stress_pybind_ctypes_consistency(simple_cell, lj_potential):
    ...
    ctypes_lib_path = os.path.join(
        project_root, "src", "thermoelasticsim", "lib", "libstress_calculator.so"
    )
    ...
    positions = np.array(
        [atom.position for atom in simple_cell.atoms], dtype=np.float64
    ).flatten()
    velocities = np.array(
        [atom.velocity for atom in simple_cell.atoms], dtype=np.float64
    ).flatten()
    forces = np.array(
        [atom.force for atom in simple_cell.atoms], dtype=np.float64
    ).flatten()
    masses = np.array([atom.mass_amu for atom in simple_cell.atoms], dtype=np.float64)
    volume = simple_cell.volume
    box_lengths = np.array(
        [simple_cell.lattice_vectors[i, i] for i in range(3)], dtype=np.float64
    )
