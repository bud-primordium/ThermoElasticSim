...
from thermoelasticsim.utils.utils import KB_IN_EV, EV_TO_GPA
...
def create_aluminum_fcc_4x4x4() -> Cell:
    ...
    a = 4.045
    nx = ny = nz = 4
    lattice = np.array([[a*nx, 0, 0], [0, a*ny, 0], [0, 0, a*nz]], dtype=np.float64)
    atoms: List[Atom] = []
    atom_id = 0
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                for p in pos_list:
                    atoms.append(Atom(id=atom_id, symbol="Al", mass_amu=26.9815, position=np.array(p)))
                    atom_id += 1
...
def run_langevin_preheat(cell: Cell, pot: EAMAl1Potential, T: float, logger: logging.Logger) -> Dict[str, List[float]]:
    ...
    hist = {"t_ps": [], "T": [], "E": []}
    logger.info(f"Langevin 预热: γ={gamma} ps^-1, {steps}步 ({steps*dt/1000:.1f} ps)")
    for s in range(steps):
        scheme.step(cell, pot, dt)
        if s % 200 == 0:
            t_ps = s*dt/1000.0
            ke = sum(0.5*atom.mass*np.dot(atom.velocity, atom.velocity) for atom in cell.atoms)
...
def run_mtk_npt(cell: Cell,
                ...
                *,
                ...
                steps: int = 20000,
                sample_every: int = 50,
                ma_window_ps: float = 0.5) -> Dict[str, Any]:
    ...
    for s in range(steps):
        ...
        if (s % (2000)) == 0:
            logger.info(f"NPT {s:6d}: T={temps[-1]:.1f}K, P={pressures_gpa[-1]:+.3f} GPa, V={volumes[-1]:.2f} Å³")

    # 压力演化图
    t = np.array(t_ps)
    P = np.array(pressures_gpa)
    T_hist = np.array(temps)
    V_hist = np.array(volumes)
    ...
    ax.plot(t, P_ma, color="#0072B2", linewidth=2.0, label=f"移动平均 (~{ma_window_ps:.1f} ps)")
    ax.axhline(y=P_target_GPa, color="#D55E00", linestyle="--", label=f"目标压力 {P_target_GPa} GPa")
    ax.set_xlabel("时间 (ps)")
    ...
    return {
        ...
        "avg_pressure_GPa": float(np.nanmean(P_ma[np.isfinite(P_ma)])) if len(P_ma)>0 else float(np.mean(P)),
        "std_pressure_GPa": float(np.nanstd(P_ma[np.isfinite(P_ma)])) if len(P_ma)>0 else float(np.std(P)),
    }
...
def nhc_sample_stress(cell: Cell,
                      pot: EAMAl1Potential,
                      T: float,
                      *,
                      dt: float = 0.5,
    ...
    nhc = NoseHooverChainPropagator(target_temperature=T, tdamp=tdamp, tchain=3, tloop=1)
    stress_calc = StressCalculator()
...
def fit_elastic_constant(strains: np.ndarray, stresses: np.ndarray) -> Tuple[float, np.ndarray]:
...
def main():
    ...
    npt_stats = run_mtk_npt(
        cell, pot, target_T, target_P, logger, run_dir,
        dt=0.15, tdamp=50.0, pdamp=150.0, steps=20000, sample_every=50, ma_window_ps=0.5
    )
    logger.info(
    ...
    strain_set = {
        "C11": {"mode": "xx", "magnitudes": np.array([-0.005, -0.004, -0.003, -0.002, 0.002, 0.003, 0.004, 0.005])},
        "C12": {"mode": "xx", "magnitudes": np.array([-0.005, -0.004, -0.003, -0.002, 0.002, 0.003, 0.004, 0.005])},
        # C44 剪切应变的 magnitude 以工程剪切 γ 表示，apply_strain 内部已除以2写入应变张量
        "C44": {"mode": "yz", "magnitudes": np.array([-0.005, -0.004, -0.003, -0.002, 0.002, 0.003, 0.004, 0.005])},
    }
    ...
    def do_series(name: str, mode: str, mags: np.ndarray) -> Dict[str, Any]:
        ...
        for m in mags:
            ...
            stat = nhc_sample_stress(dcell, pot, target_T,
                                     dt=0.5, tdamp=50.0,
                                     pre_steps=6000, prod_steps=20000, sample_every=50)
    ...
    res_C11 = do_series("C11", strain_set["C11"]["mode"], strain_set["C11"]["magnitudes"])
    res_C12 = do_series("C12", strain_set["C12"]["mode"], strain_set["C12"]["magnitudes"])
    res_C44 = do_series("C44", strain_set["C44"]["mode"], strain_set["C44"]["magnitudes"])
    ...
    def plot_series(name: str, data: Dict[str, Any]) -> str:
        ...
        ax.errorbar(xs*100.0, ys, yerr=es, fmt="o", capsize=4, color="#1f77b4", label="采样点")
        if np.isfinite(Cval) and len(xs) >= 2:
            xf = np.linspace(xs.min(), xs.max(), 100)
            yf = np.polyval(coeffs, xf)
            ax.plot(xf*100.0, yf, "--", color="#D55E00", label=f"线性拟合: {name}={Cval:.1f} GPa")
        ax.grid(True, alpha=0.3)
        if name == "C44":
    ...
    # 6) 控制台总结
    def fmt(x: Any) -> str:
        return "nan" if x is None or (isinstance(x, float) and (np.isnan(x) or np.isinf(x))) else f"{x:.1f}"
    logger.info("-"*60)
